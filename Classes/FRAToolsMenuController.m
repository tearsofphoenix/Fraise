...
VASingletonIMPDefault(FRAToolsMenuController)
...
- (void)buildInsertSnippetMenu
{
	...
	for (VASnippetCollection *collection in collectionEnumerator)
    {
		...
		for (VASnippet *snippet in array)
        {
			...
			NSString *keyString;
			if ([snippet shortcutMenuItemKeyString] != nil)
            {
				keyString = [snippet shortcutMenuItemKeyString];
			} else {
				keyString = @"";
			}
			...
			[subMenuItem setKeyEquivalentModifierMask: [snippet shortcutModifier]];
...
- (void)buildRunCommandMenu
{
	...
	NSArray *collectionEnumerator = [VACommandCollection allCommandCollections];
	...
	for (VACommandCollection *collection in collectionEnumerator)
    {
		...
		NSMutableArray *array = [NSMutableArray arrayWithArray:[[collection commands] allObjects]];
		[array sortUsingSelector:@selector(localizedCaseInsensitiveCompare:)];
